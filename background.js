...
function buildPrompt(sourceText) {
  ...
  //
  // 논문에서는 단순화가 곧 왜곡이 될 수 있어 Leichte Sprache의 강한 단순화 중
  // 위험한 규칙은 완화했다. 연구의 숫자는 그대로 두고, 단서와 한계는 지키게 했다.
  // Nachrichtenleicht가 표현의 폭을 지키려고 Leichte 대신 Einfache Sprache를
  // 택한 것과 같은 판단이다.
  //
  // 실제 논문 단락(다크패턴 선행연구)을 변환해 원문과 한 줄씩 대조한 결과를 반영했다.
  // - '특히'가 '다만'으로 바뀜: 규칙 예시로 '다만'을 적어 둔 탓. 예시를 빼고
  //   원문의 연결어를 바꾸지 말라는 규칙을 더했다
  // - '증가하면서'가 '그래서'로 바뀌어 원인과 결과가 생김: 연결어 추가 금지
  // - '모호하여'가 '모호한 편'으로 약해지고 '등'이 빠짐: 주장을 세게도 약하게도 바꾸지 않게 함
  // - 원문에 없는 약칭(FTC)과 외래어 원어(Platform)를 덧붙임: 원어 병기를
  //   다른 언어 원문일 때로 한정
  // - '인터페이스 조작(Interface Interference)'이 '인터페이스(Interface) 조작'으로 망가짐
  // - 문장을 나누다 '관점을 고려한 가이드라인'의 꾸밈 관계가 끊김
  // - 핵심 주제어 '다크패턴'이 사전에서 빠지고, 사전 뜻풀이가 본문과 어긋남
  //
...
아래 [원문]은 논문의 일부입니다. 이것을 고등학생이 처음 읽어도 이해할 수 있는 글로 바꾸어 주세요.
요약하지 말고, 내용을 줄이지 않은 채 쉽게 다시 쓰는 것이 목표입니다.
쉽게 쓰는 것보다 원문의 뜻을 정확히 지키는 것이 더 중요합니다.

[문장 쓰기]
- 한 문장에는 한 가지 내용만 담습니다.
- 한 문장을 쓴 뒤에는 반드시 줄을 바꿉니다. 한 줄에 두 문장을 쓰지 않습니다.
- 문장을 짧게 씁니다.
- 문장을 나눌 때 꾸며 주는 관계는 끊지 않습니다. 'ㄱ을 고려한 ㄴ이 필요하다'를 'ㄱ을 고려해야 한다. ㄴ이 필요하다'처럼 두 개의 다른 주장으로 쪼개지 않습니다.
- 능동으로 씁니다. 주어를 억지로 만들어 어색해진다면 자연스러운 한국어를 택합니다.
- 어려운 한자어와 딱딱한 표현은 쉬운 말로 바꿉니다.
- 비유, 관용구, 사자성어, 반어법을 쓰지 않습니다.
- 부정문보다 긍정문으로 씁니다. 이중 부정은 쓰지 않습니다.
- '~습니다' 체로 씁니다.

[원문의 뜻 지키기]
- 원문의 연결어는 뜻이 같은 말로만 옮깁니다. '특히'(강조), '그러나'(반대), '따라서'(결론), '또한'(추가)의 관계를 다른 관계로 바꾸지 않습니다.
- 원문에 없는 연결어를 덧붙이지 않습니다. 특히 '그래서', '때문에'를 넣어 원문에 없던 원인과 결과를 만들지 않습니다.
- 주장의 세기를 바꾸지 않습니다. '관련이 있다'를 '때문이다'로 세게 바꾸지 않고, '모호하다'를 '모호한 편이다'로 약하게 바꾸지도 않습니다.
- '~일 가능성이 있다', '일부', '주로', '약', '등' 같은 단서와 범위를 나타내는 말을 지우지 않습니다.
- 조건과 예외를 나타내는 표현은 빠뜨리지 않고 따로 한 문장으로 떼어 씁니다.
- 연구에 나온 숫자(참가자 수, 비율, 기간, 수치)와 인용 번호([1] 같은 표시)는 바꾸지 않고 그대로 씁니다.
- 원문에 없는 내용, 해석, 평가, 설명을 덧붙이지 않습니다.
- 같은 대상은 늘 같은 단어로 부릅니다. 다른 말로 바꾸어 쓰지 않습니다.

[용어와 이름]
- 학술 용어와 전문 용어는 쉬운 말로 바꾸지 말고 그대로 씁니다.
- 원문에 괄호로 적힌 원어는 그 용어에 붙은 그대로 남깁니다. 원어를 줄이거나 다른 낱말에 옮겨 붙이지 않습니다.
...
- 줄임말과 약자는 원문에 뜻이 적혀 있을 때만 풀어서 씁니다.
...
- 뜻풀이가 본문의 내용과 어긋나지 않게 씁니다. 본문이 그 낱말을 쓰는 뜻에 맞춰 풉니다.
- 형식 예시:
  • 낱말: 쉬운 뜻풀이 설명
